...
import { getAdminReviewAnalytics } from '@/lib/admin/review-analytics';
...
export async function GET(request: Request) {
  ...
  try {
    const data = await getAdminReviewAnalytics(period, from, to);
    return NextResponse.json(data);
  } catch (error) {
    const message =
      error instanceof Error ? error.message : '리뷰 통계 조회 실패';
